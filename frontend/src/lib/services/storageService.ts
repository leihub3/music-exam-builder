...
import type { FileUpload } from '@/lib/api/upload'

class StorageService {
  /**
   * Upload audio file for listening questions
   */
  async uploadAudioFile(file: FileUpload, questionId: string) {
    try {
   ...
   */
  async uploadNotationFile(file: FileUpload, questionId: string) {
    try {
      const fileName = `${questionId}/${uuidv4()}-${file.originalname}`
   ...
   */
  async uploadStudentSubmission(file: FileUpload, attemptId: string, questionId: string, userId: string) {
    try {
   ...
   */
  async uploadInstitutionAsset(file: FileUpload, institutionId: string) {
    try {
