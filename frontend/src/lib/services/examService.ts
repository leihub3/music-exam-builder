...
class ExamService {
   ...
   * Create a new exam
   */
  async createExam(examData: any, creatorId: string) {
   ...
   */
  async getExamById(examId: string, includeQuestions = true) {
   ...
   */
  async getInstitutionExams(institutionId: string) {
    const { data, error } = await supabaseAdmin
      .from('exams')
   ...
   */
  async publishExam(examId: string, isPublished: boolean) {
  ...
  async calculateTotalPoints(examId: string) {
    ...
    const total = allQuestions 
      ? allQuestions.reduce((sum: number, q: any) => sum + (q.points || 0), 0)
      : 0
   ...
   */
  async assignExamToStudents(examId: string, studentIds: string[], assignedBy: string, dueDate: string | null = null) {
   ...
   */
  async getStudentAssignedExams(studentId: string) {
    ...
    const publishedAssignments = (assignments || []).filter(
      (assignment: any) => assignment.exam && assignment.exam.is_published === true
    )
